...
def prune_vgg16_conv_layer(model, layer_index, filter_index, use_cuda=False):
    _, conv = list(model.features._modules.items())[layer_index]       # list of layers in the model 
    next_conv = None
    offset = 1
    print(list(model.features._modules.items()))
    # print(conv.weight.size())
    # print(conv)

    # Check the layer index that is in of bound
    ...
    # Transform the data in torch.tensor to numpy
    old_weights = conv.weight.data.cpu().numpy()
    new_weights = new_conv.weight.data.cpu().numpy()
    bias_numpy = conv.bias.data.cpu().numpy()

    # Delete the filter_index at chossen conv (at layer_index)
    new_weights[: filter_index, :, :, :] = old_weights[: filter_index, :, :, :]
    new_weights[filter_index : , :, :, :] = old_weights[filter_index + 1 :, :, :, :]
    new_conv.weight.data = torch.from_numpy(new_weights)
    
    # Push data to GPU (with Cuda)
    if use_cuda:
        new_conv.weight.data = new_conv.weight.data.cuda()
    

    # Present, this bias has (pre - 1) shape because count of (filter == biases) 
